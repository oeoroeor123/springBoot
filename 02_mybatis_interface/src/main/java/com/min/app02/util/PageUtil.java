...
public class PageUtil {
  ...
  public void setPaging(int page, int display, int count) {
    ...
    int pagePerBlock = 10; // 한 블록에 10 페이지 표시
    pageCount = (int)Math.ceil((double)count / display); // 전체 페이지 수 (Math.ceil()를 사용해 나머지가 있으면 올림 처리함)
   ...
   */
  public String getPaging(String requestURI, String... params) throws Exception { // param 여러개 
    
    if(params.length % 2 == 1)  // 파라미터는 짝수로 넘어와야 함
      throw new RuntimeException("잘못된 파라미터 형식입니다."); // 홀수일 경우 예외 발생
